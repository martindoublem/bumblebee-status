...
class Module(bumblebee.engine.Module):
    ...
    def update(self, widgets):
        try:
            ...
            self._playback = str(spotify_iface.Get('org.mpris.MediaPlayer2.Player', 'PlaybackStatus'))
            self._song = self._format.format(
                album=str(props.get('xesam:album')),
                title=str(props.get('xesam:title')),
                artist=','.join(props.get('xesam:artist')),
                trackNumber=str(props.get('xesam:trackNumber')),
                playbackStatus=u"\u25B6" if self._playback == "Playing" else u"\u258D\u258D" if self._playback == "Paused" else "",
